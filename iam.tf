...
data "aws_iam_policy_document" "ecs_task_execution_role_asssume_role_policy" {
  statement {
    actions = [
      "sts:AssumeRole"
    ]
...
resource "aws_iam_role" "ecs_task_execution_role" {
  ...
  assume_role_policy = data.aws_iam_policy_document.ecs_task_execution_role_asssume_role_policy.json
}
